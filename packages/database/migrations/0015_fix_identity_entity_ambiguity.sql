-- Fix PL/pgSQL ambiguity between the local `entity_id` variable and
-- entity_aliases.entity_id used in ON CONFLICT targets.
--
-- Migration 0009 may already be applied on existing installations, so this
-- replacement must live in a new migration rather than editing history only.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM identity_entity_links AS l
  ...
  IF source_account_id IS NOT NULL THEN
    ...
    WHERE sa.id = source_account_id
      AND sa.household_id = identity_row.household_id;
  END IF;

  account_owner := COALESCE(identity_row.member_id, account_owner);
  entity_visibility := CASE
    WHEN account_owner IS NULL THEN 'family'::visibility_scope
    ELSE 'private'::visibility_scope
  END;
  ...
  RETURNING entities.id INTO v_entity_id;
...
-- Repair any WhatsApp identities that predate the fix and do not yet have
-- their person entity. The function is idempotent.
DO $$
DECLARE
  v_identity_id uuid;
BEGIN
  FOR v_identity_id IN
    ...
    LEFT JOIN identity_entity_links AS l ON l.identity_id = i.id
    WHERE i.provider = 'whatsapp'
      AND i.kind = 'whatsapp_jid'
      AND l.identity_id IS NULL
  LOOP

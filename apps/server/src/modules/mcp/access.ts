...
export async function createMcpAccessToken(
  ...
  const label = normalizeLabel(input.label ?? "Local MCP client");
  ...
  const result = await db.query<{
    ...
    `INSERT INTO mcp_access_tokens(
     ...
     ) VALUES ($1, $2, $3, $4, ARRAY['read']::text[], $5)
     RETURNING id, label, scopes, created_at, expires_at, last_used_at, revoked_at`,
    [principal.householdId, principal.memberId, label, hashToken(token), expiresAt],
  );
...
export async function authenticateMcpToken(token: string): Promise<AuthPrincipal | null> {
  ...
  if (!value.startsWith("sol_mcp_") || value.length < 30) return null;
  const result = await db.query<{
    token_id: string;
    household_id: string;
    member_id: string;

...
function assertExpectedManifest(actual: SolPluginManifest, expected?: InstallPackageOptions["expectedManifest"]): void {
  if (!expected) return;
  if (actual.schemaVersion !== expected.schemaVersion || !sameStrings(actual.capabilities, expected.capabilities) || !sameStrings(actual.requires, expected.requires) || actual.id !== expected.id || actual.name !== expected.name || actual.version !== expected.version || !sameStrings(actual.permissions, expected.permissions)) {

...
async function authenticate(request: IncomingMessage, response: ServerResponse): Promise<SolPluginRuntimePrincipal | null> {
...
function requirePermission(response: ServerResponse, principal: SolPluginRuntimePrincipal, permission: string): boolean {
...
function availableToolScopes(principal: SolPluginRuntimePrincipal): RuntimeMcpScope[] {
  const scopes: RuntimeMcpScope[] = [];
  if (principal.permissions.includes("mcp.invoke.read")) scopes.push("read");
  if (principal.permissions.includes("mcp.invoke.actions")) scopes.push("actions");
  return scopes;
}

export async function handlePluginRuntimeApi(path: string, request: IncomingMessage, response: ServerResponse): Promise<boolean> {
  ...
  if (connectionPath) return await handlePluginConnectionsApi(path, request, response, principal);
  if (credentialPath) return await handlePluginCredentialRuntime(path, request, response, principal);
  ...
  if (path === "/v1/plugin-api/identities/person" && request.method === "POST") {
    ...
    const input = await jsonBody<{ externalId?: unknown; label?: unknown; metadata?: unknown; autoLinkMember?: unknown }>(request, response);
    if (!input) return true;
    try { sendJson(response, 200, { person: await upsertPluginPersonIdentity(principal, input) }); }
    catch (error) { runtimeError(response, error); }
    return true;
  }
  ...
  if (path === "/v1/plugin-api/identities/link" && request.method === "POST") {
    ...
    try {
      sendJson(response, 200, { link: await linkPluginIdentityToCanonicalPerson(principal, input.identityId, input.personEntityId) });
    } catch (error) { runtimeError(response, error); }
    return true;
  }
  ...
  if (path === "/v1/plugin-api/mcp/tools/available" && request.method === "GET") {
    const scopes = availableToolScopes(principal);
    if (!scopes.length) {
      sendJson(response, 403, { error: "plugin_permission_required", permission: "mcp.invoke.read_or_actions" });
      return true;
    }
  ...
  if (path === "/v1/plugin-api/mcp/tools/invoke-read" && request.method === "POST") {
    ...
    if (!input) return true;
    try { sendJson(response, 200, await invokePluginRuntimeTool(principal, "read", input)); }
    catch (error) { runtimeError(response, error); }
    return true;
  }

  if (path === "/v1/plugin-api/mcp/tools/invoke-action" && request.method === "POST") {
    if (!requirePermission(response, principal, "mcp.invoke.actions")) return true;
    const input = await jsonBody<{ name?: unknown; arguments?: unknown }>(request, response);
    if (!input) return true;
    try { sendJson(response, 200, await invokePluginRuntimeTool(principal, "actions", input)); }
    catch (error) { runtimeError(response, error); }
    return true;
  }

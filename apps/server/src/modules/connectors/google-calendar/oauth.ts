...
import { openGoogleCredential, sealGoogleCredential } from "./crypto.js";
...
async function readCredential(sourceAccountId: string): Promise<StoredCredential | null> {
  const result = await db.query<{ encrypted_payload: string }>(
    "SELECT encrypted_payload FROM google_oauth_credentials WHERE source_account_id = $1",
    [sourceAccountId],
  );
  const row = result.rows[0];
  if (!row) return null;
  const text = await openGoogleCredential(row.encrypted_payload, credentialAad(sourceAccountId));
  ...
  await db.query(
    `INSERT INTO google_oauth_credentials(source_account_id, encrypted_payload, granted_scopes, updated_at)
     ...
     DO UPDATE SET encrypted_payload = EXCLUDED.encrypted_payload,
                   granted_scopes = CASE WHEN cardinality(EXCLUDED.granted_scopes) > 0 THEN EXCLUDED.granted_scopes ELSE google_oauth_credentials.granted_scopes END,
                   updated_at = now()`,
    [sourceAccountId, encrypted, scopes ?? []],
  );
}
...
async function tokenRequest(params: URLSearchParams): Promise<TokenResponse> {
  ...
  if (!response.ok || body.error) {
    throw new Error(body.error_description || body.error || `Google OAuth token exchange failed (${response.status})`);
  }
  return body;
}
...
export async function completeGoogleOAuth(
  ...
  return {
    ...
    redirectAfter: row.redirect_after || "/calendar",
  };
}
...
export async function clearGoogleOAuth(sourceAccountId: string): Promise<void> {
  await db.query("DELETE FROM google_oauth_credentials WHERE source_account_id = $1", [sourceAccountId]);
}

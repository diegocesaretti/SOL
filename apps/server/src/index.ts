...
import { createMember, MemberValidationError, type NewMemberRole } from "./modules/identity/members.js";
...
function requestUrl(request: IncomingMessage): URL { return new URL(request.url ?? "/", "http://sol.local"); }
function pathname(request: IncomingMessage): string { return requestUrl(request).pathname; }
...
async function jsonBody<T>(request: IncomingMessage, response: ServerResponse): Promise<T | null> {
  ...
  catch (error) { sendJson(response, 400, { error: error instanceof Error ? error.message : "invalid request body" }); return null; }
}
async function principalFor(request: IncomingMessage, response: ServerResponse): Promise<AuthPrincipal | null> {
  const principal = await authenticateRequest(request);
  if (!principal) sendJson(response, 401, { error: "unauthenticated" });
  return principal;
}
function canManageMembers(principal: AuthPrincipal): boolean { return principal.role === "owner" || principal.role === "adult"; }
...
const legacyPageRedirects = new Map<string, string>([
  ["/whatsapp", "/services"],
  ["/calendar", "/services"],
  ["/home-assistant", "/services"],
  ["/mercadolibre", "/services"],
  ["/sol-whatsapp", "/services"],
  ["/executive", "/outputs"],
  ["/people", "/life?tab=people"],
  ["/projects", "/life?tab=projects"],
]);
...
async function handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (request.method === "GET") {
    const legacy = legacyPageRedirects.get(path);
    if (legacy) { redirect(response, legacy); return; }
    if (path === "/") { sendHtml(response, 200, renderOnboardingPage()); return; }
    if (path === "/services") { sendHtml(response, 200, renderPluginsPage()); return; }
    if (path === "/inputs") { sendHtml(response, 200, renderInputsPage()); return; }
    if (path === "/outputs") { sendHtml(response, 200, renderOutputsPage()); return; }
  ...
  if (request.method === "GET" && path === "/health") {
    ...
    sendJson(response, database ? 200 : 503, { ok: database, service: "sol-core", database });
    return;
  }
  if (request.method === "GET" && path === "/v1/system") {
    const database = await checkDatabase();
    sendJson(response, 200, {
      name: "SOL", architecture: "family-first data/knowledge OS + MCP", version: "0.13.0", database,
  ...
  if (request.method === "GET" && path === "/v1/onboarding") { sendJson(response, 200, await getOnboardingState()); return; }
  if (request.method === "POST" && path === "/v1/onboarding") {
    const input = await jsonBody<BootstrapInput>(request, response); if (!input) return;
    try {
      ...
      if (error instanceof AlreadyConfiguredError) { sendJson(response, 409, { error: error.message }); return; }
      if (error instanceof ValidationError) { sendJson(response, 400, { error: error.message }); return; }
      throw error;
    }
    return;
  }
  if (request.method === "POST" && path === "/v1/auth/login") {
    const input = await jsonBody<{ householdId?: string; loginName?: string; password?: string }>(request, response); if (!input) return;
    if (!input.householdId || !input.loginName || !input.password) { sendJson(response, 400, { error: "householdId, loginName and password are required" }); return; }
    const login = await loginMember(input.householdId, input.loginName, input.password);
    if (!login) { sendJson(response, 401, { error: "invalid_credentials" }); return; }
    response.setHeader("set-cookie", sessionCookie(login.token, login.maxAgeSeconds));
    sendJson(response, 200, { member: login.principal }); return;
  }
  if (request.method === "GET" && path === "/v1/auth/me") {
    const principal = await principalFor(request, response); if (!principal) return;
    sendJson(response, 200, { member: principal }); return;
  }
  ...
  if (path === "/v1/source-accounts") {
    const principal = await principalFor(request, response); if (!principal) return;
    if (request.method !== "GET") { sendJson(response, 405, { error: "source_accounts_are_plugin_managed" }); return; }
    const canSeeAll = principal.role === "owner" || principal.role === "adult";
    sendJson(response, 200, { sourceAccounts: await listSourceAccounts(principal.householdId, principal.memberId, canSeeAll) }); return;
  }

  const membersMatch = path.match(/^\/v1\/households\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\/members$/i);
  if (membersMatch) {
    const principal = await principalFor(request, response); if (!principal) return;
    const householdId = membersMatch[1];
    if (!householdId || principal.householdId !== householdId) { sendJson(response, 403, { error: "forbidden" }); return; }
    if (request.method === "GET") { sendJson(response, 200, { members: await listMembers(householdId) }); return; }
    if (request.method === "POST") {
      if (!canManageMembers(principal)) { sendJson(response, 403, { error: "forbidden" }); return; }
      const input = await jsonBody<{ displayName?: string; loginName?: string; password?: string; role?: NewMemberRole; locale?: string; timezone?: string }>(request, response); if (!input) return;
      if (!input.role || !canCreateRole(principal, input.role)) { sendJson(response, 403, { error: "role_not_allowed" }); return; }
      try {
        const member = await createMember({ householdId, displayName: input.displayName ?? "", loginName: input.loginName ?? "", password: input.password ?? "", role: input.role, locale: input.locale, timezone: input.timezone });
        sendJson(response, 201, { member });
      } catch (error) {
        if (error instanceof MemberValidationError) { sendJson(response, 400, { error: error.message }); return; }
        throw error;
      }
      return;
    }
  ...
  if (request.method === "GET" && request.headers.accept?.includes("text/html")) {
    sendHtml(response, 404, solPage("home", "No encontrado", `<section class="page"><div class="eyebrow">SOL · 404</div><h1>Esta página no existe.</h1><p class="lead">El enlace puede pertenecer a una versión anterior de SOL. Usá la navegación actual para volver a una superficie válida.</p><div class="cluster"><a class="button primary" href="/">Inicio</a><a class="button" href="/services">Servicios</a><a class="button" href="/life">Life</a></div></section>`));
    return;
  }
...
server.listen(config.port, config.host, () => {
  ...
  startWindowsTray(); outboxDispatcher.start();
  console.log("External sources and provider tools are plugin-only; install providers from Services.");
  if (config.host !== "127.0.0.1" && config.host !== "localhost") console.warn("SOL is listening beyond localhost. Use HTTPS and review member authentication before exposing it broadly.");
});

async function shutdown(signal: string): Promise<void> {
  console.log(`Received ${signal}; shutting down SOL Core`);
  stopWindowsTray(); outboxDispatcher.stop(); unregisterCandidateProcessor();
